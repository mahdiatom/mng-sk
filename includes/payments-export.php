...
function sc_export_finance_receivables_to_excel() {
    ...
    $wallet_sql = "SELECT m.id AS member_id, MIN(w.created_at) AS created_at, ABS(MIN(w.balance_after)) AS amount, m.first_name, m.last_name
        FROM $wallet_table w
...
function sc_export_finance_cashflow_to_excel() {
    ...
    $cash_in_store_raw = $include_store_cashflow ? sc_finance_sum_store_orders_items_total($from, $to, 0, 0) : 0.0;
...
function sc_export_finance_ledger_to_excel() {
    ...
    $rows = array_merge($income_rows ?: [], $store_income_rows ?: [], $manual_income_rows ?: [], $expense_rows ?: []);
