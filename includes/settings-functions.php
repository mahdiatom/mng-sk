...
function sc_get_penalty_minutes() {
    return (int)sc_get_setting('penalty_minutes', '60');
}

/**
 * Check if penalty test mode is enabled
 */
function sc_is_penalty_test_mode() {
    return (int)sc_get_setting('penalty_test_mode', '0') === 1;
}
...
function sc_get_invoice_interval_minutes() {
    return (int)sc_get_setting('invoice_interval_minutes', '43200'); // پیش‌فرض: 30 روز = 43200 دقیقه
}
...
function sc_calculate_penalty($invoice_id) {
    ...
    // حالت تست: بلافاصله جریمه اعمال می‌شود
    if (sc_is_penalty_test_mode()) {
    ...
    $minutes_passed = floor(($current_date - $created_date) / 60); // محاسبه دقیقه
...
function sc_check_and_apply_penalties() {
    ...
    // در حالت تست، تمام invoiceهای pending را بررسی می‌کنیم
    if (sc_is_penalty_test_mode()) {
        ...
        $penalty_minutes = sc_get_penalty_minutes();
        // دریافت تمام صورت حساب‌های pending که جریمه اعمال نشده و زمان گذشته >= penalty_minutes
