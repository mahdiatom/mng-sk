<?php
/**
 * هسته لایسنس AtomClub — مقادیر key، product_id و product_base را از پنل فروشنده لایسنس تنظیم کنید.
 */
if(!class_exists("atomclub_Base")) {
	class atomclub_Base {
    	public $key = "9E331285C17DFAD2";
    	private $product_id = "1";
    	private $product_base = "atomclub";
    	private $server_host = "https://www.atomclubapp.ir/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_atomclub_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=atomclub_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..","atomclub").'</h1>');
