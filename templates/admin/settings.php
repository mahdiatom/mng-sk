...
if (isset($_POST['sc_save_settings']) && check_admin_referer('sc_settings_nonce', 'sc_settings_nonce')) {
    if ($current_tab === 'penalty') {
        ...
        $penalty_minutes = isset($_POST['penalty_minutes']) ? absint($_POST['penalty_minutes']) : 60;
        $penalty_amount = isset($_POST['penalty_amount']) ? floatval($_POST['penalty_amount']) : 500;
        $penalty_test_mode = isset($_POST['penalty_test_mode']) ? 1 : 0;
        ...
        sc_update_setting('penalty_test_mode', $penalty_test_mode, 'penalty');
        
        echo '<div class="notice notice-success is-dismissible"><p>تنظیمات جریمه با موفقیت ذخیره شد.</p></div>';
    } elseif ($current_tab === 'invoice') {
        $invoice_interval_minutes = isset($_POST['invoice_interval_minutes']) ? absint($_POST['invoice_interval_minutes']) : 43200;
...
$penalty_test_mode = sc_is_penalty_test_mode();
        ...
        <?php if ($current_tab === 'penalty') : ?>
            <form method="POST" action="">
                ...
                <table class="form-table">
                    ...
                    <tr>
                        <th scope="row">
                            <label for="penalty_minutes">مدت زمان قبل از اعمال جریمه (دقیقه)</label>
                        </th>
                        <td>
                            <input type="number" 
                                   ...
                                   class="regular-text" 
                                   required>
                            <p class="description">تعداد دقیقه‌ای که بعد از ایجاد صورت حساب، در صورت عدم پرداخت، جریمه اعمال می‌شود. (مثلاً 60 = 1 ساعت، 1440 = 1 روز)</p>
                        </td>
                    </tr>
                    
                    <tr>
                        <th scope="row">
                            <label for="penalty_test_mode">حالت تست</label>
                        </th>
                        <td>
                            <input type="checkbox" 
                                   name="penalty_test_mode" 
                                   id="penalty_test_mode" 
                                   value="1" 
                                   <?php checked($penalty_test_mode, 1); ?>>
                            <label for="penalty_test_mode">فعال کردن حالت تست (در این حالت جریمه بلافاصله اعمال می‌شود)</label>
                            <p class="description">در حالت تست، جریمه بلافاصله بعد از ایجاد صورت حساب اعمال می‌شود (بدون در نظر گیری مدت زمان).</p>
                        </td>
                    </tr>
        ...
        <?php elseif ($current_tab === 'invoice') : ?>
            <form method="POST" action="">
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="invoice_interval_minutes">مدت زمان فاصله بین صورت حساب‌های تکراری (دقیقه)</label>
                        </th>
                        <td>
                            <input type="number" 
                                   ...
                                   class="regular-text" 
                                   required>
                            <p class="description">تعداد دقیقه‌ای که بین ایجاد صورت حساب‌های تکراری فاصله وجود دارد. (مثلاً 43200 = 30 روز، 1440 = 1 روز، 60 = 1 ساعت)</p>
                        </td>
                    </tr>
                </table>
