...
document.addEventListener('DOMContentLoaded', function() {
    // داده‌های ماهیانه برای نمودار
    const monthlyData = <?php echo json_encode($monthly_data); ?>;
    
    // نمودار درآمد و هزینه‌ها
    const chartCtx = document.getElementById('incomeExpensesChart');
    if (chartCtx) {
        new Chart(chartCtx, {
            type: 'bar',
            data: {
                ...
                datasets: [
                    {
                        label: 'درآمد',
                        data: monthlyData.map(m => parseFloat(m.income)),
                    ...
                    {
                        label: 'هزینه',
                        data: monthlyData.map(m => parseFloat(m.expenses)),
